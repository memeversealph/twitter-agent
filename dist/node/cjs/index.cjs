...
function parseTweetV2ToV1(tweetV2, includes, defaultTweetData) {
  ...
  if (includes?.polls?.length) {
    ...
    parsedTweet.poll = {
      id: poll.id,
      end_datetime: poll.end_datetime ? poll.end_datetime : defaultTweetData?.poll?.end_datetime ? defaultTweetData?.poll?.end_datetime : undefined,
...
async function fetchHomeTimeline(count, seenTweetIds, auth) {
  ...
  const tweets = entries.map((entry) => entry.content.itemContent?.tweet_results?.result).filter((tweet) => tweet !== undefined);
  return tweets;
}
